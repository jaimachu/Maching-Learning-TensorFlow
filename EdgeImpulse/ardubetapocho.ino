...
/* Includes ---------------------------------------------------------------- */
#include <jaimachu-project-1_inferencing.h>
...
void setup()
{
    ...
    ei_printf("Configuración del modelo:\n");
    ei_printf("\tResolución de la imagen: %dx%d\n", EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT);
    ei_printf("\tTamaño del frame: %d\n", EI_CLASSIFIER_DSP_INPUT_FRAME_SIZE);
    ei_printf("\tNúmero de clases: %d\n", sizeof(ei_classifier_inferencing_categories) / sizeof(ei_classifier_inferencing_categories[0]));

    Wire.begin(8);        // Inicializar la comunicación I2C y asignar dirección 8
    Serial.begin(115200);   // Inicializar la comunicación serial
    Wire.onReceive(receiveEvent);  // Configurar la función para recibir datos
...
*/
void hacerFoto(){
        ei_printf("\nEmpezando a clasificar en 2 segundos...\n");
        ei_sleep(2000);
        ei_printf("Tomando la foto...\n");

        // Configuración de la cámara
        if (ei_camera_init() == false) {
            ei_printf("ERROR: Fallo al inicializar el sensor de imagen\r\n");
        }
        ei_printf("LLEGAMOS..\n");
        uint32_t resize_col_sz;
        uint32_t resize_row_sz;
        ...
        if (res) {
            ei_printf("ERROR: Fallo al calcular las redimensiones (%d)\r\n", res);
        }
        ...
        // Ejecutamos el modelo para clasificar el objeto de la foto
        ei_impulse_result_t result = { 0 };
        EI_IMPULSE_ERROR ei_error = run_classifier(&signal, &result, debug_nn);
        if (ei_error != EI_IMPULSE_OK) {
            ei_printf("Fallo al ejecutar el modelo (%d)\n", ei_error);
            ei_free(snapshot_mem);
        }

        // Predicciones
        ei_printf("Predicción (DSP: %d ms., Clasificación: %d ms., Anomalía: %d ms.): \n",
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
          ...
          const char *tag = result.classification[ix].label;
          if (max_value < value){
            max_value = value;
            etiqueta = tag;
          }    
        ...
        ei_printf(" \nClasificado:  %s: %.5f\n", etiqueta,
                  max_value);
        int num_tag;
        if (strcmp(etiqueta, "Tuerca") == 0) {
            num_tag = 2;
        } 
        else if (strcmp(etiqueta, "Arandela") == 0) {
            num_tag = 3;
        } 
        else if (strcmp(etiqueta, "Mariposa") == 0) {
            num_tag = 4;
        }
        else{
          num_tag = 1;
        }
        ei_printf(" \Número de la etiqueta:  %d\n", num_tag);
        resultado = num_tag;
...
void loop(){
    ...
    recibido = false;
    ei_printf("Hemos terminado la primera iteración");
        }
}
void requestEvent(){
...
void receiveEvent(int bytes) {
  /*while (Wire.available()) {    // Si hay datos disponibles en el buffer
    int n = Wire.read();
    recibido = true;
  }*/
    String message = "";
